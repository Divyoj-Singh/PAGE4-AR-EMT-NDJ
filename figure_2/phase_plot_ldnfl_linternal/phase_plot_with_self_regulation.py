# -*- coding: utf-8 -*-
"""
Created on Sun Oct 11 00:29:33 2020
...
import os
import time
#%% Specify the number of CPU cores here:
no_of_cpu_for_use=55
cores=mp.cpu_count()
...
def finding_state(k,lambda2,time, p):
    ''' this returns the state of the system, whether it is mono(1), bistable (2) or oscillatory (6)'''
    p['lAtoX'] = lambda2
    p['lXtoA'] = lambda2
    # consider two trajectories staring from opposite iniital conditions to check bistability
    PM1,PH1,C1,PU1,A1,X1 = integrate_cons(time, p, 200, 0)
    tm1, index1, extr1 = find_extremal(time, A1)
    PM2,PH2,C2,PU2,A2,X2 = integrate_cons(time, p, 0, 200)
...
T = 10*(24*7) ## simulating for 10 weeks (multipied by 24*7 to convert to hours since t has inherent unit in hours because all perimeters are in /hour)
dt = 0.001
...
for m in lambda_array:
    t=time.time()
    p['lXtoX']=m
    filename="self_regulation="+str(p['lXtoX'])
    print(filename)
    ## maake two vectors for x axis and y axis points for making grid
    v1 = np.arange(0.0,1.01,0.005)
    v1=np.around(v1,2)
    v2 = np.arange(0.0,1.01,0.005)
    v2=np.around(v2,2)
    # defining 2d array
    ...
    for i in range(v1.size ):
    	...
    	## starting a cpu paraallised loop for an array:
        output= pool.starmap_async(finding_state, [(k, j, tm, p) for k, j in enumerate(v2)]).get()
        for l in output:
            ampl[i][l[0]]=l[1] 
            pool.close()        
    phase_plot_data_frame=pd.DataFrame(data=ampl,columns=v2,index=v1) # 2d array to dataframe
    phase_plot_data_frame=phase_plot_data_frame[::-1] #reordering dataframe so that 0 is at left corner
    #%% Save data and plot:
    ...
    ax.set_xlabel("lamda dnfl")
    ax.set_ylabel("lambda internal coupling (page4)")
