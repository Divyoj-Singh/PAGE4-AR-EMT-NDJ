# -*- coding: utf-8 -*-
"""
Created on Wed Feb 10 21:28:28 2021
...
folder="./Time_period/AtoZ=0.1_ZtoA=0.1/Sic=160000/"
...
# df=pd.read_csv(folder+'Time_period_'+str(parameter_dictionary['Sic'])+'.csv')

A1,W1= integrate_cons(seed_value,tm, p)
tm1, index1, extr1 = find_extremal(tm, A1)
wt_amplitude=tm1[2]-tm1[0] ## difference between two maximas
...
for parameter_id in p:
    ...
    plus_amplitude=tm2[2]-tm2[0]## difference between two maximas
    ...
    minus_amplitude=tm3[2]-tm3[0]
    df.at[parameter_id,'-10%']=((minus_amplitude-wt_amplitude)/wt_amplitude)*100
    
    #%% plotting dynamics: 
    f, ax = plt.subplots(figsize=(3.6,2.7))
    sns.set_theme(style="whitegrid")        
    ax.plot(tm/(24*7),W1/1000,'k-',label = 'wt')# , label = 'IC1'
    ax.plot(tm/(24*7),W2/1000,'b-',label = '+10%') #, label = 'IC2'
    ax.plot(tm/(24*7),W3/1000,'r-',label = '-10%') #, label = 'IC2'
    
    ax.tick_params(axis='y',labelsize=10,rotation=90)
    ax.tick_params(axis='x',labelsize=10)
    ax.set_ylabel("mir200 (1000 molecules)",fontsize=12)
...
df.to_csv(folder+'Time_period_'+str(parameter_dictionary['Sic'])+'.csv')    
...
ax.set_xlabel("% Change in Time Period of AR",fontsize=10)
...
plt.savefig(folder+'Time_period_'+str(parameter_dictionary['Sic'])+'.jpeg',dpi=2000)
