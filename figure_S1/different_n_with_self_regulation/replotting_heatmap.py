...
for filename in os.listdir(folder+"data/"):
    ...
    n_DNFL_AR=filename[:-4].split("_")[3]
    n_PAGE4_AR=filename[:-4].split("_")[7]
    ...
    ax.set_ylabel("$\lambda_{PAGE4}$",fontsize=8)
    ax.set_xlabel("$\lambda_{DNFL}$",fontsize=8)
    ax.set_title("("+n_DNFL_AR+","+ n_PAGE4_AR +")")       
    plt.subplots_adjust(top=0.85, bottom=0.25, left=0.25, right=0.95, hspace=0.25,wspace=0.5)
    plt.savefig(folder+"plots/"+filename[:-4]+".png",dpi=1000)    
    plt.close()